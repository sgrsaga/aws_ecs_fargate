...
resource "aws_iam_role" "codeBuildRoleECS" {
  name               = "codeBuildRoleECS"
  assume_role_policy = data.aws_iam_policy_document.assume_role.json
}

data "aws_iam_policy_document" "codeBuildRoleECSPolicyDoc" {
  statement {
    effect = "Allow"
  ...
  statement {
    ...
    condition {
      ...
      values = aws_subnet.private_subnets.ids
    }
...
resource "aws_iam_role_policy" "codeBuildRoleECSPolicy" {
  role   = aws_iam_role.codeBuildRoleECS.name
  policy = data.aws_iam_policy_document.codeBuildRoleECSPolicyDoc.json
}
...
resource "aws_codebuild_project" "codebuild" {
  ...
  build_timeout = "5"
  ## HARD Coded role
  service_role  = aws_iam_role.codeBuildRoleECS.arn
